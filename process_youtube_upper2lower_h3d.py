...
import smplx
import subprocess
from lom.utils.load_checkpoint import load_pretrained_vae_compositional, load_pretrained_lm, load_pretrained_vae_compositional_h3d
from moviepy.editor import VideoFileClip, AudioFileClip
from lom.data.mixed_dataset.scripts.motion_process import (process_file, recover_from_ric)
...
def main():
    ...
    if cfg.TEST.CHECKPOINTS:
        ...
        load_pretrained_vae_compositional_h3d(cfg, model, logger, phase="demo")
    ...
    upper_root_dir = '/simurgh/group/juze/datasets/YouTube_Talking/TOKENS_AGENT_25/upper'
    ...
    for upper_name in tqdm(upper_paths, desc="Processing upper paths"):
        save_path_lower = os.path.join(upper_root_dir.replace('upper', 'lower'), upper_name)
        if os.path.exists(save_path_lower):
            continue
        ...
        rec_index_all_hands = []
        # num_subdivision = 1
        for index in tqdm(range(num_subdivision), desc="Processing upper chunks"):
        ...
        Path(save_path_lower).parent.mkdir(parents=True, exist_ok=True)
        np.save(save_path_lower, rec_index_lower.cpu().numpy())

        if save_reconstruced:

            save_path_reconstructed = os.path.join(upper_root_dir.replace('TOKENS_AGENT_25', 'Reconstructed_lower').replace('upper', ''), upper_name).replace('.npy', '.npz')
            if os.path.exists(save_path_reconstructed):
                continue
            ...
            rec_y_trans = smooth_gaussian(rec_y_trans, k=500)         # Gaussian smoothing
            ...
            Path(save_path_reconstructed).parent.mkdir(parents=True, exist_ok=True)
            np.savez( save_path_reconstructed,
                        betas=rec_beta_np,
                        poses= rec_pose_np,
