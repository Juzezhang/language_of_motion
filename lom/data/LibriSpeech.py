...
from .humanml import Text2MotionDatasetEval, Text2MotionDataset, Text2MotionDatasetCB, MotionDataset, MotionDatasetVQ, Text2MotionDatasetToken, Text2MotionDatasetM2T, LibriSpeechDataset, LibriSpeechDatasetEval
from .utils import librispeech_collate


class LibriSpeechDataModule(BASEDataModule):
    def __init__(self, cfg, **kwargs):

        super().__init__(collate_fn=librispeech_collate)
        self.cfg = cfg
        ...
        # Path to the dataset
        # data_root = cfg.DATASET.LIBRISPEECH.ROOT
        # 路径设置
        self.hparams.data_root = cfg.DATASET.LIBRISPEECH.ROOT
        self.hparams.token_root = cfg.DATASET.LIBRISPEECH.TOKEN_ROOT
        self.hparams.used_trainset = cfg.DATASET.used_trainset  # e.g. 'train-clean-100'
        self.hparams.used_testset = cfg.DATASET.used_testset  # e.g. 'test-clean'

        # 数据集设置
        self.hparams.sample_rate = 16000  # 设置采样率
        self.hparams.batch_size = cfg.TRAIN.BATCH_SIZE
        # 额外的参数
        ...
        self.DatasetEval = LibriSpeechDatasetEval
