...
class GPTLosses(BaseLosses):
    ...
    def __init__(self, cfg, stage, num_joints, **kwargs):
        ...
        if stage == "vae":
            ...
            losses.append("recons_rec-loss")
            params['recons_rec-loss'] = 1.0
            losses.append("recons_velocity-loss")
            params['recons_velocity-loss'] = 1.0

            losses.append("recons_acceleration-loss")
            params['recons_acceleration-loss'] = 1.0

            losses.append("recons_vectices-loss")
            params['recons_vectices-loss'] = 1.0

            losses.append("recons_vertices-vel-loss")
            params['recons_vertices-vel-loss'] = 1.0

            losses.append("recons_vertices-acc-loss")
            params['recons_vertices-acc-loss'] = 1.0

            losses.append("recons_commit-loss")
            params['recons_commit-loss'] = 1.0
    ...
    def update(self, rs_set):
        ...
        if self.stage in ["vae"]:
            # total += sum(value for key, value in rs_set.items() if "loss" in key)
            for key, value in rs_set.items():
                if "loss" in key:
                    # 使用 _update_loss 更新每个损失值
        ...
        if self.stage in ["lm_pretrain", "lm_instruct", "lm_causal_instruct"]:
            total += self._update_loss("gpt_loss", rs_set['outputs'].loss, rs_set['outputs'].loss)
