...
def calculate_kid(real_activations, generated_activations):
    ...
    return results
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # Specify using GPU 1

# Define the file paths
gt_path = "/nas/nas_32/AI-being/zhangjz/exp_motion/paper_result/vq_abalation/mgpt_ds4/result/gt/"
res_path = "/nas/nas_32/AI-being/zhangjz/exp_motion/paper_result/vq_abalation/mgpt_ds4/result/rec/"

print("Ground truth path:", gt_path)
print("Result path:", res_path)

# Retrieve all corresponding file names
gt_files = sorted(glob(os.path.join(gt_path, "*.npy")))
res_files = sorted(glob(os.path.join(res_path, "*.npy")))

assert len(gt_files) == len(res_files), "Mismatch in the number of GT and RES files!"

# Initialize metric lists
mpjpe_list, pampjpe_list, accel_list = [], [], []
...
for gt_file, res_file in tqdm(zip(gt_files, res_files), total=len(gt_files)):
    ...
    gt_data = gt_data.squeeze(0)  # Convert shape to (B, J, 3)
    res_data = res_data.squeeze(0)

    # Get batch size (B)
    B_gt = gt_data.shape[0]
    B_res = res_data.shape[0]

    # Take the smaller batch size
    B = min(B_gt, B_res)

    # Trim both tensors to have the same batch size
    gt_data = gt_data[:B]
    res_data = res_data[:B]

    # Convert to PyTorch tensors and move to GPU
    gt_joints_tensor = torch.tensor(gt_data, dtype=torch.float32).cuda()
    res_joints_tensor = torch.tensor(res_data, dtype=torch.float32).cuda()

    # Compute MPJPE (Mean Per Joint Position Error)
    mpjpe = calc_mpjpe(res_joints_tensor * 1000, gt_joints_tensor * 1000).mean().item()  # Convert to mm
    mpjpe_list.append(mpjpe)

    # Compute PA-MPJPE (Procrustes-Aligned MPJPE)
    pampjpe = calc_pampjpe(res_joints_tensor * 1000, gt_joints_tensor * 1000).mean().item()
    pampjpe_list.append(pampjpe)

    # Compute ACCEL (Acceleration Error)
    accel = calc_accel(res_joints_tensor * 1000, gt_joints_tensor * 1000).mean().item()
    accel_list.append(accel)

# Print average results
print("Average MPJPE:", np.mean(mpjpe_list))
print("Average PA-MPJPE:", np.mean(pampjpe_list))
print("Average ACCEL:", np.mean(accel_list))