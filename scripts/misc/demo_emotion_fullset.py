...
def audio_token_to_string(audio_token, lengths, codebook_size=500):
    audio_string = []
    len(audio_token)
    for i in range(len(audio_token)):
        ...
        audio_string.append(
            (f'<audio_id_{codebook_size}>' +
             ''.join([f'<audio_id_{int(i)}>' for i in audio_list]) +
             f'<audio_id_{codebook_size + 1}>'))
    return audio_string
...
def load_example_input_audio(audio_path, task, model):
    audio_tokens = []
    audio_lengths = []
    file_name = []
    # audio_path_list = os.listdir(audio_path)
    for audio_file in audio_path:
        if audio_file.startswith('M'):
            continue
        audio_token = np.load(join('/nas/nas_32/AI-being/zhangjz/exp_motion/datasets/beat2/audios_token/', audio_file+'.npy'))[::2]
        audio_token = torch.tensor(audio_token)
        audio_tokens.append(audio_token)
        audio_lengths.append(audio_token.shape[0])
        file_name.append(audio_file)
    audio_token_string = audio_token_to_string(audio_tokens, audio_lengths)

    # Strips the newline character
    motion_joints = [torch.zeros((1, 1, 22, 3))] * len(audio_tokens)
    motion_lengths = [0] * len(audio_tokens)
    motion_token_string = ['']
    motion_token = torch.zeros((1, 263))
    ...
    return_dict = {
        ...
        'audio_token_string': audio_token_string,
    }
...
def main():
    ...
    if cfg.DEMO.EXAMPLE:
        ...
        id_list = []
        with cs.open(cfg.DEMO.EXAMPLE, "r") as f:
            for line in f.readlines():
                id_list.append(line.strip())

        return_dict, file_name = load_example_input_audio(id_list, task, model)
        audio_token_string, in_joints = return_dict['audio_token_string'], return_dict['motion_joints']

    batch_size = 64
    if audio_token_string:
        for b in tqdm(range(len(audio_token_string) // batch_size + 1)):
            audio_batch = audio_token_string[b * batch_size:(b + 1) * batch_size]
            in_joints_batch = in_joints[b * batch_size:(b + 1) * batch_size]
            save_name = file_name[b * batch_size:(b + 1) * batch_size]
            batch = {
                "length":
                return_dict["audio_lengths"][b * batch_size:(b + 1) *
                                              batch_size],
                "audio":
                audio_batch
            }
            ...
            for i in range(len(joints)):
                ...
                np.save(os.path.join(output_dir, f'{save_name[i]}_out.npy'), xyz)
                # np.save(os.path.join(output_dir, f'{save_name[i]}_in.npy'), xyz_in)

                # with open(os.path.join(output_dir, f'{save_name[i]}_in.txt'), 'w') as f:
                #     f.write(audio_batch[i])
                #
                # with open(os.path.join(output_dir, f'{save_name[i]}_out.txt'), 'w') as f:
                #     f.write(output_texts[i])

                # pose_vis = plot_3d.draw_to_batch(xyz_in, [''], [os.path.join(output_dir, f'{save_name[i]}_in.gif')])
                # pose_vis = plot_3d.draw_to_batch(xyz, [''], [os.path.join(output_dir, f'{save_name[i]}_out.gif')])
