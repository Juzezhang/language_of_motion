...
def main():
    ...
    for config in cfg.DATASET.datasets:
        ...
        if dataset_name == "AMASS":
            ...
            output_dir_amass = os.path.join(data_root, 'reconstructed_motion_ds4')
        ...
        if dataset_name == "BEAT2":
            ...
            output_dir_beat2 = os.path.join(data_root, 'reconstructed_motion_ds4')
    ...
    for data_index in tqdm(range(len(path_name_list)), desc=f'motion tokenize'):
        ...
        try:
            if dataset_name == 'amass':
                ...
                gt_npz = np.load(pjoin(data_root, 'amass_data_align', path_name.split('.')[0] + ".npz"), allow_pickle=True)
            else:
                data_root = beat2_root
                output_dir = output_dir_beat2
                gt_npz = np.load(pjoin(data_root, 'smplxflame_30', path_name.split('.')[0] + ".npz"), allow_pickle=True)
            hand_token = np.load(pjoin(data_root, 'TOKENS_DS4_PaperVersion', 'hands', path_name))
            hand_token = torch.from_numpy(hand_token).to('cuda').int()
            upper_token = np.load(pjoin(data_root, 'TOKENS_DS4_PaperVersion', 'upper', path_name))
            upper_token = torch.from_numpy(upper_token).to('cuda').int()
            lower_token = np.load(pjoin(data_root, 'TOKENS_DS4_PaperVersion', 'lower', path_name))
        ...
        if dataset_name == 'amass':
            ...
            face_token = np.load(pjoin(data_root, 'TOKENS_DS4_PaperVersion', 'face', path_name))
